...
import { Send, FileText, ExternalLink, History, RefreshCcw, Quote } from "lucide-react";
import ReactMarkdown from "react-markdown";
import remarkGfm from "remark-gfm";

import { generateReport, getReport, listReports, getReportLineage, listProjectProviderSettings } from "@/lib/api/client";
import type { ReportResult, ReportListItem, ReportLineage, ProviderSettingData } from "@/lib/api/types";
...
export function ReportViewer() {
  ...
  const [reportType, setReportType] = useState<"research_brief" | "summary" | "comparison">("research_brief");
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    setReport(null);
    setLineage(null);
    const toastId = toast.loading(`Generating ${reportType.replace("_", " ")} with ${provider}...`);
    try {
        ...
        <div className="flex flex-col gap-6 lg:col-span-1">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Build a multi-source research report based on retrieved and synthesized evidence.
              </CardDescription>
            </CardHeader>
            <CardContent>
              <form onSubmit={handleSubmit} className="flex flex-col gap-3">
                ...
                <p className="text-xs text-muted-foreground mt-1">
                  This process runs insight synthesis first, then renders a full Markdown document with lineage tracking.
                </p>
                <Button type="submit" disabled={busy || !activeProjectId || !query} className="w-full gap-2 mt-2">
                  {busy ? <Spinner size="sm" /> : <FileText className="h-4 w-4" />}
              ...
              {history.length === 0 ? (
                ...
                <div className="flex flex-col">
                  {history.map((item) => (
                    <button
                      key={item.report_id}
                      onClick={() => handleLoadReport(item.report_id)}
                      disabled={busy}
          ...
          {!report && !busy ? (
            <Card className="flex h-[300px] items-center justify-center border-dashed">
              <div className="flex flex-col items-center gap-2 text-muted-foreground">
                ...
                <p>Generate a report to view the formatted Markdown output.</p>
              </div>
            </Card>
          ) : busy && !report ? (
            <Card className="flex h-[300px] items-center justify-center">
              <div className="flex flex-col items-center gap-4 text-muted-foreground">
                <Spinner size="lg" />
                <p>Synthesizing insights and drafting report...</p>
              </div>
            </Card>
          ) : report ? (
            <>
                ...
                <Card>
                  <CardHeader>
                    <div className="flex items-center gap-2">
                      ...
                      <Badge variant="outline" className="ml-auto">{lineage.source_ids.length} sources</Badge>
                    </div>
                    <CardDescription>
                      This report was generated using verified chunks from {lineage.source_ids.length} indexed source(s).
                    </CardDescription>
                  </CardHeader>
                  <CardContent className="p-0">
                    <div className="grid gap-px bg-border">
                      {lineage.source_ids.map((sourceId) => (
                        <div key={sourceId} className="flex items-center justify-between bg-card px-6 py-4">
                           <p className="text-sm font-mono text-muted-foreground">Source ID: {sourceId}</p>
                           <a href={`/sources?id=${sourceId}`} className="text-xs text-primary hover:underline">
                             View details
                           </a>
                        </div>
                      ))}
