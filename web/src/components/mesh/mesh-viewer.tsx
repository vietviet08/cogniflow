...
export function MeshViewer() {
  ...
  async function loadHistory() {
    ...
    try {
      ...
      const meshReports = response.data.items.filter(
        (item) => item.type === "conflict_mesh",
      );
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    const toastId = toast.loading(
      `Generating Knowledge Mesh with ${provider}...`,
    );
    try {
        ...
        <div className="flex flex-col gap-6 lg:w-[320px] shrink-0">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Map out concepts, relationships, and contradictions from your
                evidence.
              </CardDescription>
            </CardHeader>
            <CardContent>
              <form onSubmit={handleSubmit} className="flex flex-col gap-3">
                ...
                <div className="flex flex-col gap-1.5 mt-2">
                  <Label htmlFor="query-input">Focus Area</Label>
                  <Textarea
                    id="query-input"
                    required
                    rows={3}
        ...
        <div className="lg:col-span-4 flex-1 flex flex-col">
          {report?.structured_payload ? (
            <div className="h-[82vh] min-h-[600px] border border-border rounded-xl overflow-hidden bg-background">
              <MeshGraph
                payload={report.structured_payload as ConflictMeshPayload}
              />
            </div>
          ) : (
            <Card className="flex h-full min-h-[400px] items-center justify-center text-center">
              <CardContent>
                <p className="text-sm text-muted-foreground">
                  {busy
                    ? "Generating Graph... This may take a minute."
                    : "Generate or load a Mesh to view the interactive graph."}
                </p>
              </CardContent>
            </Card>
          )}
