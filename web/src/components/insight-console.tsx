...
export function InsightConsole() {
  ...
  async function handleExportInsights() {
    ...
    try {
      ...
      const blob = new Blob([JSON.stringify(response.data.items, null, 2)], {
      ...
      toast.error(
        error instanceof Error ? error.message : "Failed to export insights.",
        {
          id: toastId,
        },
      );
    ...
    >
      {!canMutateProject && activeProjectId ? (
        <div className="rounded-md border border-amber-300/40 bg-amber-500/10 px-4 py-3 text-sm text-amber-900 dark:text-amber-200">
          You have viewer access for this project. Insight generation is
          disabled.
        </div>
      ) : null}
        ...
        <div className="flex flex-col gap-6 lg:col-span-1">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Ask a broad research question to extract themes and findings
          ...
          {!insight && !busy ? (
            <Card className="flex h-[300px] items-center justify-center border-dashed">
              <div className="flex flex-col items-center gap-2 text-muted-foreground">
                ...
                <p>
                  Run an analysis or select one from history to view findings.
                </p>
              </div>
            </Card>
          ) : busy && !insight ? (
                      ...
                      {insight.citations.map((citation, index) => (
                        ...
                        >
                          <p className="text-sm font-medium">
                            {citation.title || "Unknown Document"}
                          </p>
                          <p
