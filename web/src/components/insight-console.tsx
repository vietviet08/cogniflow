...
import { Send, BookOpen, Quote, ExternalLink, History, RefreshCcw } from "lucide-react";

import { generateInsight, getInsight, listInsights, listProjectProviderSettings } from "@/lib/api/client";
import type { CitationData, InsightResult, InsightListItem, ProviderSettingData } from "@/lib/api/types";
...
export function InsightConsole() {
  ...
  useEffect(() => {
    ...
    let ignore = false;
    listProjectProviderSettings(activeProjectId).then((response) => {
      if (!ignore) setProviderSettings(response.data.items);
    }).catch(() => {
      if (!ignore) setProviderSettings([]);
    });
    ...
    >
      <div className="grid gap-6 lg:grid-cols-3">
        ...
        <div className="flex flex-col gap-6 lg:col-span-1">
          <Card>
            <CardHeader>
              <CardTitle className="text-base">Generate Insight</CardTitle>
              <CardDescription>
                Ask a broad research question to extract themes and findings across many documents.
              </CardDescription>
            </CardHeader>
            <CardContent>
              <form onSubmit={handleSubmit} className="flex flex-col gap-3">
                <div className="flex flex-col gap-1.5">
                  <Label htmlFor="provider-select">Synthesizer Provider</Label>
                  <select
                    id="provider-select"
                    value={provider}
                ...
                <div className="flex flex-col gap-1.5">
                  <Label htmlFor="query-input">Research Question</Label>
                  <Textarea
                    id="query-input"
                    required
                    rows={4}
                ...
                <Button type="submit" disabled={busy || !activeProjectId || !query} className="w-full gap-2 mt-2">
          ...
          <Card>
            <CardHeader className="flex flex-row items-center justify-between py-4">
              ...
              <Button variant="ghost" size="icon" onClick={loadHistory} disabled={loadingHistory || !activeProjectId}>
          ...
          {!insight && !busy ? (
            <Card className="flex h-[300px] items-center justify-center border-dashed">
              <div className="flex flex-col items-center gap-2 text-muted-foreground">
                ...
                <p>Run an analysis or select one from history to view findings.</p>
              </div>
            </Card>
          ) : busy && !insight ? (
            <Card className="flex h-[300px] items-center justify-center">
              <div className="flex flex-col items-center gap-4 text-muted-foreground">
                <Spinner size="lg" />
                <p>Gathering evidence and synthesizing insights...</p>
              </div>
            </Card>
          ) : insight ? (
            <>
              <Card>
                <CardHeader>
                  <div className="flex items-center gap-2">
                    <CardTitle className="text-lg">Synthesis Overview</CardTitle>
                ...
                <CardContent className="p-6">
                  {insight.findings.length === 0 ? (
                    <p className="text-sm text-muted-foreground italic">No specific findings were extracted.</p>
                  ) : (
                    <div className="flex flex-col gap-6">
                      {insight.findings.map((finding, idx) => (
                        <div key={idx} className="flex flex-col gap-2">
                          <h4 className="text-base font-semibold">{finding.theme}</h4>
                          <ul className="list-inside list-disc space-y-1">
                            {finding.points.map((pt, i) => (
                ...
                <Card>
                  <CardHeader>
                    <div className="flex items-center gap-2">
                      ...
                      <Badge variant="outline" className="ml-auto">{insight.citations.length}</Badge>
                    </div>
                  </CardHeader>
                  <CardContent className="p-0">
                    <div className="grid gap-px bg-border">
                      {insight.citations.map((citation, index) => (
                        <div key={`${citation.chunk_id}-${index}`} className="flex flex-col gap-1 bg-card px-6 py-4">
                          <p className="text-sm font-medium">{citation.title || "Unknown Document"}</p>
                          <p className="text-xs font-mono text-muted-foreground truncate" title={citation.chunk_id}>
